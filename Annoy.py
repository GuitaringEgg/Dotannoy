...
class Annoy():
    ...
    sites = ["sickipedia-random", "sickipedia-top-100", "anti-jokes-random", "wikipedia-random", "oatmeal-random"]
    ...
    def run(self, args):
        # Set logging level
        if True:
            log.basicConfig(filename="dotannoy.log", filemode="a", format="%(levelname)s: %(message)s", level=log.DEBUG)
        ...
        if not args.default_puns and args.download:
            n = args.items if args.items else 100
            if args.download == self.sites[0]:
                self.sickipedia_random(n)
            elif args.download == self.sites[1]:
                self.sickipedia_top_100()
            elif args.download == self.sites[2]:
                self.anti_joke_random(n)
            elif args.download == self.sites[3]:
                self.wikipedia_random(n)
            elif args.download == self.sites[4]:
                self.oatmeal_random(n)
            else:
    ...
    def oatmeal_random(self, n):
        o = open(self.inf, "w")

        jokes = []

        # Download the required number of jokes from the site by scraping the random page
        data  = urllib2.urlopen('http://theoatmeal.com/djtaf/')

        parsed_html = BeautifulSoup(data.read())
        part1 = parsed_html.body.find_all('h2', attrs={'class':'part1'})
        i = 0
        for joke in part1:
            if i >= n%112 and n%112 == 0:
                break
            part2 = parsed_html.body.find('h2', id="part2_{}".format(i))
            jokes.append("{} {}".format(joke.text.encode("ascii", "ignore").strip(), part2.text.encode("ascii", "ignore").strip() if part2 else "").replace(chr(13), " ").replace("\n", " "))
            i += 1

        log.info("Downloaded {} jokes".format(len(jokes)))
